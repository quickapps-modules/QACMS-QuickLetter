<?php
class UsersController extends QuickLetterAppController {
    var $name = 'Users';
    var $uses = array('QuickLetter.User', 'QuickLetter.Group', 'QuickLetter.Cfield', 'QuickLetter.Sending', 'QuickLetter.Habtm');
    var $helpers = array('QuickLetter.Cfield');
    var $components = array('QuickLetter.Csv');
        ...
        switch ($action):
            ...
            case 'import':
                ...
                foreach ($csv_lines as $line) {
                    ...
                    if ($this->User->validates()) {
                        $oks[] = sprintf(__t("Successfully inserted [%s] into %s groups"), $data[$i]['User']['email'], count($data[$i]['Group']['Group']));
    ...
    function export($download = false) {
        ...
        if ($download) {
            ...
            if ($File->exists()) {
                ...
                header("Content-Disposition: attachment; filename=mod_newsletters-export-".date(__t('Y-m-d')).".csv");
        ...
        if ($this->data) {
            #mini validation
                ...
                if (!in_array('id', $this->data['Export']['Fields'])) {
                    $errors['id'] = __t('The subscriber id must be present in all exports, you cannot de-select this field.');
                }

                if (!isset($this->data['Export']['Group']) || count($this->data['Export']['Group']) == 0) {
                    $errors['groups'] = __t('You must select at least one group.');
                }

                if (!isset($this->data['Export']['enclosed']) || empty($this->data['Export']['enclosed'])) {
                    $errors['enclosed'] = __t('Encloser symbol can not be empty.');
                }

                if (!isset($this->data['Export']['delimited']) || empty($this->data['Export']['delimited'])) {
                    $errors['delimited'] = __t('Delimiter symbol can not be empty.');
                }
