<?php
class GroupsController extends ModNewslettersAppController {
	var $uses = array('ModNewsletters.Group', 'ModNewsletters.Habtm', 'ModNewsletters.User');
	var $name = 'Groups';
	...
	function delete($form = false){
		...
		if ( isset($this->data['delete_form']) && isset($this->data['Group']['id']) ) {
			if ( isset($this->data['handle_users']) && $this->data['handle_users'] == 'delete' ) {
				$ids = $this->Habtm->find('all', array('fields' => array('user_id') ,'recursive' => -1, 'conditions' => "ModNewsletters.list_id = {$this->data['id']}") );
