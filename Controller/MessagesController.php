<?php
class MessagesController extends QuickLetterAppController {

    var $uses = array(    'QuickLetter.User', 'QuickLetter.Message', 'QuickLetter.Cfield', 'QuickLetter.Group', 'QuickLetter.Habtm', 'QuickLetter.Queue');

    var $components = array('Newsletter');

    function index() {
        /* uses:= 'QuickLetter.User'
         for($i=0; $i<100; $i++) {
         $list = rand(18,20);
         $html = rand(0,1);
         $data = array(
    ...
    function send() {
        ...
        if ($this->data['Message']['send_type'] == 'queue') {
            ...
            if (count($mailNotFound) > 0) {
                ...
                App::import('Lib', 'QuickLetter.mailer');
                ...
                foreach ($mails as $email) {
                    ...
                    $mail->Subject = QuickLetterComponent::tagReplace($this->data['Message']['subject'], $user['User']);
                    if ($user['User']['html_email '] == 1 && strlen(trim($this->data['Message']['body_html'])) > 0) { // HTML version
                        $mail->Body        = QuickLetterComponent::tagReplace($this->data['Message']['body_html']."<br/><br/><br/>".$this->data['Message']['footer'], $user['User']);
                        $mail->AltBody    = QuickLetterComponent::tagReplace($this->data['Message']['body_text']."\n\n\n".$this->data['Message']['footer'], $user['User']);
                    } else {
                        $mail->Body    = QuickLetterComponent::tagReplace($this->data['Message']['body_text']."\n\n\n".$this->data['Message']['footer'], $user['User']);
                    }
    ...
    function preview() {
        ...
        $this->data['Message']['from_field'] = $this->Newsletter->tagReplace($this->data['Message']['from_field']);
        $this->data['Message']['subject'] = $this->Newsletter->tagReplace($this->data['Message']['subject']);
        $this->data['Message']['test_mails'] = (isset($this->data['Message']['test_mails'])) ? $this->Newsletter->tagReplace($this->data['Message']['test_mails']) : null;
        $this->data['Message']['body_html'] = $this->Newsletter->tagReplace($this->data['Message']['body_html']);
        $this->data['Message']['body_text'] = $this->Newsletter->tagReplace($this->data['Message']['body_text']);
        $this->data['Message']['footer'] = $this->Newsletter->tagReplace($this->data['Message']['footer']);
    }
