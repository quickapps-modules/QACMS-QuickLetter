...
class CsvComponent extends Component {
        ...
        if (is_string($file_data)) {
            if (!empty($file_data)) {
                ...
                if  ($createCsv) {
                    ...
                    $this->errors['csvtext'] = __t("File Upload Error.");
                    return false;
                }
            } else {
                $this->errors['csvtext'] = __t("Empty csv data.");
                return false;
            }
        } else {
            App::uses('Upload', 'Lib');
            ...
            $old_mask = umask(0);
            if (!$handle->processed) {
                $this->errors['csvfile'] = __t("File Upload Error.");
                return false;
            }
            umask($old_mask);
            return true;
    ...
    function validates($upload = false) {
        ...
        ($this->controller->data['Import']['type'] == 'text' && empty($this->controller->data['Import']['csvtext']))) {
            ...
            $this->errors['csvsource'] = __t("There is no file in the filesystem to import, please try again.");
        }

        if (!isset($this->controller->data['Import']['groups']) || count($this->controller->data['Import']['groups']) == 0) {
            $this->errors['groups'] = __t("You must select at least one group to import these subscribers into under 'Imported Data Destination'.");
        }

        if (!isset($this->controller->data['Import']['fields_enclosed']) ||empty($this->controller->data['Import']['fields_enclosed'])) {
            $this->errors['fields_enclosed'] = __t("Invalid 'Enclosed By'");
        }

        if (!isset($this->controller->data['Import']['fields_delimited']) || empty($this->controller->data['Import']['fields_delimited'])) {
            $this->errors['fields_delimited'] = __t("Invalid 'Delimited By'");
        }
        if ($upload) {
