...
<h1 class="main-title"><?php __e("User's Groups"); ?></h1>
...
<table width="100%">
	...
	<tr>
		...
		<input type="text" class="text" value="<?php __e("Group Name"); ?>"
			name="addList_name" id="addList_name"
			onFocus="if(this.value == '<?php __e("Group Name");?>'){this.value = ''}"
			onBlur="if(this.value == ''){this.value = '<?php __e("Group Name");?>'}" />
		&raquo;
		<div id="lists_select" style="display: inline;"><?php echo $this->element("groups_selectbox"); ?>
		</div>
		<input class="primary_lg" type="submit" value="<?php __e("Add"); ?>" />
...
<button type="button" class="primary_lg right"
	onclick="Messaging.kill('groups_list_dialogue');"><?php __e("Close"); ?></button>
</fieldset>
</div>
</div>
</div>
