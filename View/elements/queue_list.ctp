<table width="100%" id="table_list" class="tablekit">
    <thead>
        <tr>
            ...
            <th width="15%" align="left" class="<?php echo __t(("date-iso"); //es: date-eu ?>"><?php echo __t(("Sent Date"); ?></th>
            <th width="37%" align="left" class="text"><?php echo __t(("Message Title"); ?></th>
            <th width="22%" align="left" class="text"><?php echo __t(("Groups"); ?></th>
            <th width="9%" align="left" class="number"><?php echo __t(("Progress"); ?></th>
            <th width="15%" align="left" class="text"><?php echo __t(("Status"); ?></th>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        <tr class="content" style="<?php echo ($queue['Queue']['status'] == 'Stalled') ? 'background-color:#FFD5D5;' : (   ($queue['Queue']['status'] == 'Paused') ? 'background-color:#FFF8D5;': '' ); ?>">
            ...
            <td align="left"><?php echo date( __t('Y-m-d H:i:s'), strtotime($queue['Queue']['date']) ); ?></td>
            ...
            <td align="left"><?php echo __t(($queue['Queue']['status']); echo ($queue['Queue']['status'] == 'Complete') ? " ({$queue['Queue']['total']})" : ""; ?></td>
        </tr>
        <?php
            }
        ...
        <tr>
            ...
            <?php echo __t(("You have no queues."); ?><br />
            </td>
        </tr>
        <?php }?>
    </tbody>
                ...
                <div class="wrap">
                    <div id="queue_dialogue_wrap">
                        ...
                        foreach ($stalled as $queue) {
                            echo sprintf(__t("It appears as though queue id <b>%s</b> has stalled. Please resume sending or cancel this message.<br/>"), $queue['Queue']['id']) ;
                        }
                        ?>
                    </div>
                    <fieldset class="nopad">
                        <button type="button" class="primary_lg right" onclick="Messaging.kill('queue_dialogue');"><?php echo __t(("Close"); ?></button>
                    </fieldset>
                </div>
            </div>
        </div>
